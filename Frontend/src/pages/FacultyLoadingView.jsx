...
export default function FacultyLoadingView() {
  ...
  const statusLabel = useMemo(() => {
    ...
    return preflight.status === 'blocked' ? 'Blocked' : 'Ready';
  }, [preflight]);

  const statusTone = preflight?.status === 'blocked' ? 'danger' : 'success';
  ...
  async function handleRun() {
    try {
      ...
      const generatedAt = formatDateTimeStandard(new Date());
      const summary = {
        generatedAt,
  ...
  const unassignedSubjects = result?.report?.unassigned_subjects || [];
  const facultyWithFreeUnits = result?.report?.faculty_with_free_units || [];
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-[28px] border border-white/70 bg-white/70 p-6 shadow-[0_30px_50px_rgba(75,42,184,0.08)] backdrop-blur-xl md:p-8">
        ...
        <div className="relative">
          ...
          <h2 className="mt-3 text-4xl font-headline-xl font-extrabold tracking-tight text-on-surface md:text-5xl">
            Faculty Loading Generation
          </h2>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-on-surface-variant md:text-base">
            Revamped to follow the old faculty loading flow while keeping Chronomaria’s current design system. Runs are
            validated with pre-flight checks before GA execution.
          </p>
        </div>
      </div>
      ...
      <div className="glass-panel rounded-2xl p-6">
        ...
        <div className="mt-6 space-y-5">
          {!loadingPreflight && preflight?.status === 'blocked' ? (
            <div className="rounded-2xl border border-error/20 bg-error-container/35 p-4 text-sm text-error">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-xl border border-white/60 bg-white/80 p-4">
              <p className="text-xs uppercase tracking-[0.22em] text-on-surface-variant">Max Generations</p>
              <p className="mt-2 text-lg font-bold text-on-surface">120</p>
            </div>
            <div className="rounded-xl border border-white/60 bg-white/80 p-4">
              <p className="text-xs uppercase tracking-[0.22em] text-on-surface-variant">Population Size</p>
              <p className="mt-2 text-lg font-bold text-on-surface">72</p>
            </div>
            <div className="rounded-xl border border-white/60 bg-white/80 p-4">
              <p className="text-xs uppercase tracking-[0.22em] text-on-surface-variant">Mutation Rate</p>
              <p className="mt-2 text-lg font-bold text-on-surface">0.12</p>
            </div>
            <div className="rounded-xl border border-white/60 bg-white/80 p-4">
              <p className="text-xs uppercase tracking-[0.22em] text-on-surface-variant">Max Runtime</p>
              <p className="mt-2 text-lg font-bold text-on-surface">20s</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-3 text-sm font-bold text-on-primary shadow-lg shadow-primary/20 transition-all hover:-translate-y-0.5 hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-60"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-outline-variant bg-white/80 px-4 py-3 text-sm font-semibold text-on-surface transition-colors hover:bg-white"
            >
              <ShieldAlert size={16} />
            ...
            <div className="inline-flex items-center gap-2 rounded-xl border border-white/60 bg-white/80 px-4 py-3 text-sm text-on-surface-variant">
              <Gauge size={16} />
              Status: <span className={`font-bold ${statusTone === 'danger' ? 'text-error' : 'text-emerald-700'}`}>{statusLabel}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel rounded-2xl p-6">
        <SectionHeader
          ...
          subtitle="Quick summary of the most recent execution."
          action={<div className="rounded-full bg-secondary-container/20 px-3 py-1 text-xs font-bold uppercase tracking-[0.22em] text-secondary">History</div>}
        />
        ...
        {!lastRunSummary ? (
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div className="rounded-xl border border-white/60 bg-white/80 p-4">
              <p className="text-xs uppercase tracking-[0.22em] text-on-surface-variant flex items-center gap-1"><History size={14} /> Generated At</p>
              <p className="mt-2 font-semibold text-on-surface">{lastRunSummary.generatedAt}</p>
            </div>
            <div className="rounded-xl border border-white/60 bg-white/80 p-4">
              <p className="text-xs uppercase tracking-[0.22em] text-on-surface-variant flex items-center gap-1"><WandSparkles size={14} /> Quality</p>
              <p className="mt-2 font-semibold text-on-surface">{lastRunSummary.quality}</p>
            </div>
            <div className="rounded-xl border border-white/60 bg-white/80 p-4">
              <p className="text-xs uppercase tracking-[0.22em] text-on-surface-variant flex items-center gap-1"><Bolt size={14} /> Fitness</p>
              <p className="mt-2 font-semibold text-on-surface">{lastRunSummary.fitness ?? '-'}</p>
            </div>
            <div className="rounded-xl border border-white/60 bg-white/80 p-4">
              <p className="text-xs uppercase tracking-[0.22em] text-on-surface-variant">Assignments</p>
              <p className="mt-2 font-semibold text-on-surface">{lastRunSummary.assignments ?? '-'}</p>
            </div>
            <div className="rounded-xl border border-white/60 bg-white/80 p-4">
              <p className="text-xs uppercase tracking-[0.22em] text-on-surface-variant">Persisted</p>
              <p className="mt-2 font-semibold text-on-surface">{lastRunSummary.persisted ?? '-'}</p>
            </div>
            <div className="rounded-xl border border-white/60 bg-white/80 p-4">
              <p className="text-xs uppercase tracking-[0.22em] text-on-surface-variant">Run ID</p>
              <p className="mt-2 truncate font-semibold text-on-surface">{lastRunSummary.runId || '-'}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="glass-panel rounded-2xl p-6">
          <SectionHeader
            title="Quality & Readiness Report"
            subtitle="Old-page inspired report panel, mapped to current GA payload."
            action={<div className="rounded-full bg-primary-container/15 px-3 py-1 text-xs font-bold uppercase tracking-[0.22em] text-primary">Report</div>}
          />
          ...
          <div className="mt-6 space-y-4">
            ...
            <details className="rounded-2xl border border-white/60 bg-white/80 p-4">
              <summary className="cursor-pointer text-sm font-bold uppercase tracking-[0.22em] text-on-surface">
                  ...
                  return (
                    <div key={severity}>
                      ...
                      <div className="mt-2 space-y-2">
                        {items.slice(0, 12).map((issue, idx) => (
                          <div key={`${severity}-${idx}`} className="rounded-xl border border-white/60 bg-white p-3 text-sm">
                            <p className="font-semibold text-on-surface flex items-center gap-2"><FileWarning size={14} /> {issue.problem}</p>
                            <p className="mt-1 text-xs uppercase tracking-[0.18em] text-on-surface-variant">
                              {issue.type} {issue.field ? `• ${issue.field}` : ''} {issue.id ? `• ID ${issue.id}` : ''}
                            </p>
                          </div>
                        ))}
                      </div>
        ...
        <div className="space-y-6">
          <div className="glass-panel rounded-2xl p-6">
            <SectionHeader title="Unassigned Subjects" subtitle="Subjects not loaded by GA." />
            <div className="mt-4 max-h-[230px] space-y-2 overflow-y-auto pr-1">
              {unassignedSubjects.length === 0 ? (
                <p className="rounded-xl border border-white/60 bg-white/80 p-3 text-sm text-on-surface-variant">No unassigned subjects reported.</p>
              ) : (
                unassignedSubjects.slice(0, 20).map((item, idx) => (
                  <div key={`unassigned-${idx}`} className="rounded-xl border border-white/60 bg-white/80 p-3 text-sm">
                    <p className="font-semibold text-on-surface">{item.descriptive_title || item.title || item.code || 'Subject'}</p>
                    <p className="mt-1 text-xs uppercase tracking-[0.18em] text-on-surface-variant">
                      {item.code || '-'} • {item.course_no || '-'} • Dept {item.department_id ?? '-'}
                    </p>
                  </div>
                ))
              )}
          ...
          <div className="glass-panel rounded-2xl p-6">
            <SectionHeader title="Faculty with Free Units" subtitle="Units not maximized yet." />
            <div className="mt-4 max-h-[230px] space-y-3 overflow-y-auto pr-1">
              {facultyWithFreeUnits.length === 0 ? (
                ...
                facultyWithFreeUnits.slice(0, 20).map((row, idx) => (
                  <div key={`free-${idx}`} className="rounded-xl border border-white/60 bg-white/80 p-3">
                    <div className="flex items-start justify-between gap-3 text-sm">
                      <div>
                        ...
                        <p className="text-xs uppercase tracking-[0.18em] text-on-surface-variant">{row.faculty_role || 'N/A'} • Dept {row.department_id ?? '-'}</p>
                      </div>
                      <p className="text-on-surface-variant">{row.total_units ?? 0}/{row.max_units ?? 0}</p>
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="glass-panel rounded-2xl p-6">
            <SectionHeader title="Faculty Load Balance" subtitle="Current load distribution." />
            <div className="mt-4 max-h-[260px] space-y-3 overflow-y-auto pr-1">
              {loadBalance.slice(0, 16).map((row, idx) => (
      ...
      <div className="glass-panel rounded-2xl p-6">
        <SectionHeader
          ...
          action={<div className="inline-flex items-center gap-2 rounded-full border border-white/60 bg-white/80 px-3 py-1 text-xs font-bold uppercase tracking-[0.22em] text-on-surface-variant">{assignments.length} row(s)</div>}
        />

        {!assignments.length ? (
          <div className="mt-6 rounded-2xl border border-dashed border-outline-variant/60 bg-white/60 p-6 text-sm text-on-surface-variant">
            No generated list yet. Run GA to populate assignments.
          </div>
        ) : (
                  ...
                  {assignments.map((item, index) => {
                    ...
                    return (
                      <tr key={`assignment-${index}`} className="border-t border-white/60">
                        <td className="px-4 py-3 text-on-surface">{offering.section || '-'}</td>
                        <td className="px-4 py-3 text-on-surface">{offering.code || '-'}</td>
                        <td className="px-4 py-3 text-on-surface">{offering.course_no || '-'}</td>
                        <td className="px-4 py-3 text-on-surface">{offering.descriptive_title || '-'}</td>
                        <td className="px-4 py-3 text-on-surface">{offering.units ?? '-'}</td>
                        <td className="px-4 py-3 text-on-surface-variant">{offering.mth_schedule || '-'}</td>
                        <td className="px-4 py-3 text-on-surface-variant">{offering.tfs_schedule || '-'}</td>
                        <td className="px-4 py-3 text-on-surface">{faculty.faculty_name || '-'}</td>
                      </tr>
                    );
                  })}
                </tbody>
      ...
      {error ? (
        <motion.div
          ...
          className="rounded-2xl border border-error/20 bg-error-container/40 p-4 text-sm text-error"
        >
          {error}
        </motion.div>
      ) : null}
